...
for (let i = 0; i < rows.length; i++) {
    ...
    arrayOfSymbols.push(
        Array.from(row).map((char) => char !== "." && isNaN(parseInt(char))),
    );
...
function doChecking(rowToCheck: number, columnToCheck: number) {
    if (
        ...
        if (beginning > -1 && ending > -1) {
            result += parseInt(rows[rowToCheck].slice(beginning, ending));
            for (let i = beginning; i < ending; i++) {
                visited[rowToCheck][i] = true;
            }
        } else if (ending < 0) {
            result += parseInt(rows[rowToCheck].slice(beginning, rows[0].length));
            for (let i = beginning; i < rows[0].length; i++) {
                visited[rowToCheck][i] = true;
            }
        } else {
            result += parseInt(rows[rowToCheck].slice(columnToCheck, ending));
